...
export const PaymentMethodButton = ({ icon, title, isSelected, onSelect, disabled }: PaymentMethodButtonProps) => {
  return (
    <button
      type="button"
      onClick={onSelect}
      ...
      className={`
        ...
        ${disabled
          ? 'border-default-200 opacity-50 cursor-not-allowed'
          : isSelected
            ? 'border-primary bg-primary/10 shadow-lg shadow-primary/25'
            : 'border-default-300 hover:border-primary/50 hover:shadow-md cursor-pointer'
        }
      `}
    >
      {/* Icon Container */}
