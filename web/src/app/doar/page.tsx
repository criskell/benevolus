...
import { Button, Input, Image, Chip, Card, CardBody, RadioGroup, Radio, Tabs, Tab, Modal, ModalContent, ModalHeader, ModalBody, Table, TableHeader, TableBody, TableColumn, TableRow, TableCell } from '@heroui/react';
...
const DoarPage = () => {
  ...
  const [criteria, setCriteria] = useState<'equal' | 'mixed' | 'integral'>('mixed');
  const [distributionType, setDistributionType] = useState<'category' | 'favorites'>(likedSlugs.length >= 2 ? 'favorites' : 'category');
  ...
  const distribution = useMemo(() => {
    ...
    if (criteria === 'equal') {
      const amountPer = Math.floor(totalAmount * 100 / targetCampaigns.length);
      ...
      const otherCampaigns = targetCampaigns.filter(c => c.slug !== campaign.slug);
      const amountPerOther = otherCampaigns.length > 0 ? Math.floor(remainingAmount / otherCampaigns.length) : 0;
      
      const result = [];
      result.push({ slug: campaign.slug, amount: mainAmount });
      otherCampaigns.forEach(c => result.push({ slug: c.slug, amount: amountPerOther }));
      return result;
    }
  ...
  if (!campaignSlug) {
    return <div className="container mx-auto px-4 py-8">Parâmetro &apos;campaign&apos; não fornecido na URL.</div>;
  }

  if (!campaign) {
    return <div className="container mx-auto px-4 py-8">Campanha com slug &apos;{campaignSlug}&apos; não encontrada.</div>;
  }

  if (!campaign.image) {
    return <div className="container mx-auto px-4 py-8">Campanha sem imagem.</div>;
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <Card className="mb-6">
        <CardBody>
          <h3 className="text-lg font-semibold text-primary mb-4">💡 Distribuição Inteligente</h3>
          ...
          <Tabs selectedKey={distributionType} onSelectionChange={(key) => { if (programmaticRef.current) { programmaticRef.current = false; return; } setDistributionType(key as 'category' | 'favorites'); }}>
            <Tab key="favorites" title="Favoritos" isDisabled={likedSlugs.length < 2}>
              ...
              <RadioGroup value={criteria} onValueChange={(value) => setCriteria(value as 'equal' | 'mixed' | 'integral')} className="mb-4">
                <Radio value="equal">Dividir igualmente - Mesmo valor para todas as campanhas</Radio>
                <Radio value="mixed">Distribuição inteligente - 80% para a principal, 20% dividido entre as outras</Radio>
                <Radio value="integral">Valor integral - 100% para a campanha principal</Radio>
              </RadioGroup>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {distribution.map((item) => {
                      const camp = campaignMap[item.slug];
                      return (
                        <div key={item.slug} className="flex justify-between text-sm bg-gray-50 p-2 rounded">
                    ...
                    <div className="flex justify-between font-semibold text-primary bg-gray-100 p-2 rounded mt-2">
                      <span>Total:</span>
                      <span>{formatMoney(distribution.reduce((sum, item) => sum + item.amount, 0))}</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            <Tab key="category" title="Relacionadas">
              <p className="text-sm text-gray-600 mb-4">Distribua entre campanhas de {campaign.category}</p>
              <RadioGroup value={criteria} onValueChange={(value) => setCriteria(value as 'equal' | 'mixed' | 'integral')} className="mb-4">
                <Radio value="equal">Dividir igualmente - Mesmo valor para todas as campanhas</Radio>
                <Radio value="mixed">Distribuição inteligente - 80% para a principal, 20% dividido entre as outras</Radio>
                <Radio value="integral">Valor integral - 100% para a campanha principal</Radio>
            ...
            {relatedCampaigns.slice(0, 3).map((related) => (
              <Card key={related.slug} className="hover:shadow-lg transition-shadow">
                <CardBody className="p-0">
                  ...
                  />
                  <div className="p-4">
                    <h4 className="text-sm font-medium line-clamp-2 mb-2">{related.title}</h4>
                    <Chip size="sm" variant="flat" className="mb-3">
                      {related.category}
                    </Chip>
                    <div className="flex space-x-2">
                      <Button size="sm" variant="flat" onPress={() => handleQuickDonate(related, 1000)} className="flex-1">
                        R$ 10
                      </Button>
                      <Button size="sm" variant="flat" onPress={() => handleQuickDonate(related, 2500)} className="flex-1">
                        R$ 25
                      </Button>
                    </div>
                  </div>
        ...
        <CardBody>
          ...
          {cart.cart.length === 0 ? (
            ...
            <div>
                ...
                <Table aria-label="Lista de doações" className="min-w-full">
                  ...
                  <TableBody>
                    {cart.cart.map((item, index) => (
                      <TableRow key={index}>
                        <TableCell className="truncate max-w-xs">{item.title}</TableCell>
                        ...
                        <TableCell className="text-center">
                          {editingIndex === index ? (
                            <div className="flex space-x-1">
                              <Button size="sm" color="primary" onPress={() => { cart.updateQuantity(index, editedAmount * 100); setEditingIndex(null); }}>
                                Salvar
                              </Button>
                              <Button size="sm" variant="flat" onPress={() => setEditingIndex(null)}>
                                Cancelar
                              </Button>
                            </div>
                          ) : (
                            <div className="flex space-x-1">
                              <Button size="sm" variant="flat" onPress={() => { setEditingIndex(index); setEditedAmount(item.amount / 100); }}>
                                Editar
                              </Button>
                              <Button size="sm" color="danger" variant="flat" onPress={() => cart.removeFromCart(index)}>
                                Remover
                              </Button>
                            </div>
                          )}
              ...
              <div className="flex justify-between font-semibold text-primary bg-gray-100 p-2 rounded mt-2">
                <span>Total:</span>
                <span>{formatMoney(cart.cart.reduce((sum, item) => sum + item.amount, 0))}</span>
              </div>
              <Button color="primary" className="w-full mt-4" onPress={() => setShowDonationSummary(true)}>
                Finalizar Doação
              </Button>
            </div>
          )}
      ...
      <Modal isOpen={showDonationSummary} onClose={() => setShowDonationSummary(false)} size="lg">
        <ModalContent>
          <ModalHeader>
          ...
          <ModalBody>
            <div className="text-center mb-4">
              <p className="text-sm text-gray-600 mb-2">QR Code para pagamento (exemplo)</p>
