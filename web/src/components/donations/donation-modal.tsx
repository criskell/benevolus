...
export const DonationModal = ({ campaign, isOpen, onClose }: DonationModalProps) => {
  ...
  const [amount, setAmount] = useState(10); // Default R$10
  ...
  const distributionSlugs = distributionType === 'category' ? [campaign.slug, ...relatedCampaigns.slice(0, 2).map(c => c.slug)] : likedSlugs;
  ...
  const handleQuickDonate = (relatedCampaign: Campaign, quickAmount: number) => {
    cart.addToCart({
      ...
      image: relatedCampaign.images[0],
      amount: quickAmount,
    });
  };
                    ...
                    <CardBody className="p-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Button
                          size="sm"
                          variant="light"
                          onPress={() => handleQuickDonate(related, 500)} // R$5
                        >
                          +R$5
                        </Button>
                      </div>
